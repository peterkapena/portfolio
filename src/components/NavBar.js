...
export default function NavBar() {
    ...
    const renderMobileMenu = (
        <Menu anchorEl={mobileMoreAnchorEl} anchorOrigin={{ vertical: 'top', horizontal: 'right' }}
            ...
            {leftSide.map((item, index) => <MenuItem key={index}>{item}</MenuItem>)}
        </Menu>
    );
    ...
    return (
        <div className={classes.grow}>
            <AppBar style={{ padding: '0px', borderRadius: '10px' }} position="static">
                <Toolbar style={{ padding: '0px' }}>
                    <Paper style={{ minHeight: 'inherit', borderRadius: '10px 0px 0px 10px' }} square>
                        <IconButton onClick={() => navTo('/')} style={{ borderRadius: '1px', minHeight: 'inherit', width: '60px' }}>
                    ...
                    <Divider orientation="vertical" flexItem style={{ width: '.5%' }}></Divider>

                    <IconButton style={{ marginLeft: '10px' }} onClick={toggleTheme}>
                        {theme.theme.palette.type === 'dark' ? <Brightness7></Brightness7> : <Brightness4></Brightness4>}
                    </IconButton>
